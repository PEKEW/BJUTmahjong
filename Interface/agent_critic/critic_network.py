...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_tensor, h_cur, action):
        ...
        h_next = self.act((1 - update_gate) * h_cur + update_gate * cnm)
        return h_next
...
class Critic(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        self.concentrate = nn.Sequential(nn.Linear(27, 1, bias=True))
        self.last_conv = nn.Conv1d(self.hidden_dim[-1], 1, kernel_size=3, padding=1, bias=True)
        cell_list = []
    ...
    def forward(self, input_tensor, mask=None, target=None, hidden_state=None):
        ...
        layer_output = layer_output.view(bszie * seq_len, -1, 27)
        reward = torch.tanh(self.last_conv(layer_output))
        reward = self.concentrate(torch.squeeze(reward)).view(bszie, seq_len)
        if target is not None:
            loss = ((reward - target[:, None]).pow(2) * mask).sum() / mask.sum()
            return loss
        elif mask is not None:
            out = []
            for i in range(bszie):
                length = int(mask[i, :].sum())
                out.append(torch.tensor(reward[i, length - 1]))
            reward = torch.stack(out, dim=0)
            return reward
        else:
